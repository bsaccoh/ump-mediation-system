    ...
    try:
        ...
        for r in rows:
            ...
            data.append({
                'id': r.pk,
                'name': r.name,
                'test_date': r.test_date.isoformat() if r.test_date else '',
                'region': r.region or '',
                'technology': r.technology or '4G',
                'tool_used': r.tool_used or 'TEMS',
                'operator_code': r.operator_code or 'orange',
                'operator_label': op_label,
                'operator_name': r.operator_name or 'Orange',
                'route_type': r_type,
                'status': 'COMPLETED' if r.status in ('ANALYSED', 'REPORTED') else (r.status or 'COMPLETED'),
        ...
        import traceback
        traceback.print_exc()
        return JsonResponse({'records': [], 'total': 0, 'page': 1, 'pages': 1, 'error': str(exc)}, status=500)
