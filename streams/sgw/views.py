"""SGW CDR search, export, and detail views."""
import csv
import io
...
def _build_sgw_queryset(params):
    """Build filtered SGWRecord queryset from request params dict."""
    ...
    query = SGWRecord.objects.select_related('file', 'source')
    filters = []
...
@login_required
def sgw_search(request):
    """SGW CDR search page."""
    total_records = SGWRecord.objects.count()
    sources = DataSource.objects.filter(enabled=True).order_by('name')

    apn_counts = (SGWRecord.objects.exclude(apn='')
                  .values('apn').annotate(count=Count('id'))
                  .order_by('-count')[:20])

    return render(request, 'dashboard/sgw_search.html', {
...
@login_required
def sgw_search_api(request):
    """SGW CDR search API endpoint (POST)."""
    ...
    query, _ = _build_sgw_queryset(request.POST)
    total = query.count()

    if total == 0:
        total_in_db = SGWRecord.objects.count()
        if total_in_db > 0:
            apn_counts = (SGWRecord.objects.values('apn')
            ...
            message = 'No SGW records. Upload and process SGW CDR files first.'
...
@login_required
def sgw_export(request):
    """Export filtered SGW records as CSV."""
    if request.method != 'POST':
        return JsonResponse({'error': 'POST required'}, status=405)

    query, filter_desc = _build_sgw_queryset(request.POST)
    total = query.count()
...
@login_required
def sgw_detail(request, pk):
    """SGW CDR record detail view."""
    record = get_object_or_404(SGWRecord, pk=pk)
    return render(request, 'dashboard/sgw_detail.html', {'record': record})
